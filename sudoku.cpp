...
bool number_used_in_square(int row, int col, int num, const GRID &s)
{
    // compute grid
    int gridCol = col / 3;
    int gridRow = row / 3;
    ...
    for (int row = 0; row < 3; row++)
    {
        ...
        {
            if (s[gridRow * 3 + row][gridCol * 3 + col] == num)
                return true;
        }
    }
    return false;
...
bool number_can_be_placed(int row, int col, int num, const GRID &s)
{
    ...
    return !number_used_in_col(col, num, s) &&
           ...
           !number_used_in_square(row, col, num, s);
}
bool solveSudoku(GRID &s)
{
